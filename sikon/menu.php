...
$sql  = mysql_query("SELECT akses_level FROM user WHERE user_id='$_SESSION[user_id]'");
$r    = mysql_fetch_array($sql);

echo "<p><div align='center'>Login as $_SESSION[username]<br />";
echo "<a href=core/login/logout.php>Logout</a></div></p>";
...
echo "<p class='menuHeading'>User</p>
		...
		<li><a href=$_SERVER[PHP_SELF]?component=user&act=view&id=$_SESSION[user_id]>ProfilKu</a></li>
		<li><a href=$_SERVER[PHP_SELF]?component=user&act=news>Update News</a></li>
		<li><a href=$_SERVER[PHP_SELF]?component=user&act=list>User List</a></li>
		<li><a href=../cx52d/ target=_blank>Formulir Pendaftaran</a></li>
		</ul>";
		
if ($r[akses_level] =="Panitia Lokal" OR $r[akses_level]=="Administrator") {
	echo "<p class='menuHeading'>Panitia Lokal</p>
		<ul class='menuList'>
		<li><a href=$_SERVER[PHP_SELF]?component=sekretariat&act=registrasi_ulang>Registrasi Ulang</a></li>
		...
		<li><a href=$_SERVER[PHP_SELF]?component=sekretariat&act=kelola_acara>Kelola Acara</a></li>
		<li><a href=$_SERVER[PHP_SELF]?component=sekretariat&act=kelola_konsumsi>Jadwal Konsumsi</a></li>
		<li><a href=$_SERVER[PHP_SELF]?component=sekretariat&act=absensi>Absen Acara</a></li>
		<li><a href=$_SERVER[PHP_SELF]?component=sekretariat&act=konsumsi>Absen Konsumsi</a></li>
		<li><a href=$_SERVER[PHP_SELF]?component=sekretariat&act=cetak_bukti_daftar>Cetak Bukti Pendaftaran</a></li>
		</ul>";
}
...
if ($r[akses_level] =="Pengurus Pusat" OR $r[akses_level]=="Administrator") {
	echo "<p class='menuHeading'>Pengurus Pusat</p>
		...
		<li><a href=$_SERVER[PHP_SELF]?component=ppusat&act=kredensi>Kredensi</a></li>
...
if ($r[akses_level] =="Panitia Lokal" OR $r[akses_level]=="Pengurus Pusat" OR $r[akses_level]=="Administrator") {
	echo "<p class='menuHeading'>Reports</p>
		...
		<li><a href=$_SERVER[PHP_SELF]?component=reports&act=data_pembayaran>Data Pembayaran</a></li>
		...
		<li><a href=$_SERVER[PHP_SELF]?component=reports&act=data_komisi>Data Komisi</a></li>
		<li><a href=$_SERVER[PHP_SELF]?component=reports&act=data_kpa>Data KPA</a></li>
		<li><a href=$_SERVER[PHP_SELF]?component=reports&act=data_kredensi>Data Kredensi</a></li>
		<li><a href=$_SERVER[PHP_SELF]?component=reports&act=data_acara>Data Acara</a></li>
		<li><a href=$_SERVER[PHP_SELF]?component=reports&act=data_absensi>Data Absensi</a></li>
